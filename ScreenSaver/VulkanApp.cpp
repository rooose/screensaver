...
void VulkanApp::mouseMovedCallback(GLFWwindow* window, double xpos, double ypos)
{
    glfwSetWindowShouldClose(window, GLFW_TRUE);
}

void VulkanApp::keyboardPressedCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    glfwSetWindowShouldClose(window, GLFW_TRUE);
}
...
void VulkanApp::createGraphicsPipeline() {
    ...
    // Can be destroyed as soon as the pipeline is created
...
void VulkanApp::createRenderPass() {
    ...
    colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT; // for multisampling - one sample for now
...
void VulkanApp::createComputeDescriptorSetLayout() {
    ...
    layoutBindings[1].binding = 1; // last frame
    layoutBindings[1].descriptorCount = 1;
    layoutBindings[1].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    layoutBindings[1].pImmutableSamplers = nullptr;
    layoutBindings[1].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    layoutBindings[2].binding = 2; // current frame
...
void VulkanApp::recreateSwapChain() {
    // if minimized window, pause draw calls
...
// VERTEX BUFFERS +=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=++=+=+=+
void VulkanApp::createBuffer(VkDeviceSize bufferSize, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMem) {
...
void VulkanApp::createShaderStorageBuffers() {
    ...
    void* data;
    vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
    memcpy(data, noise.data(), (size_t)bufferSize);
    vkUnmapMemory(device, stagingBufferMemory);
...
void VulkanApp::drawFrame() {
    ...
    updateUniformBuffer(currentFrame); // update time
    ...
    // record cmd buffers with what we want
...
void VulkanApp::mainLoop() {
    while (!glfwWindowShouldClose(window)) { // TODO: Change to while nothing moves - mouse/keyboard
        glfwPollEvents();
        drawFrame();
